...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.name == 'Imposter':
                QueenAnt.counter -= 1
            elif isinstance(insect, QueenAnt) and insect.name == 'Queen':
                return 
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        """
        # BEGIN Problem 3 and Problem 4
        position = self.place
        index = 0
        while position is not beehive:
            if position.bees and self.min_range<=index<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        shallow = place.bees[:]        
        bee_ri = iter(shallow)
        if place.bees[0].damage >= self.armor:
            for b in bee_ri:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        if bee is not None and self.digesting == 0:
    ...
    def action(self,colony):
        # BEGIN Problem 6 #
        return self.eat_bee(random_or_none(self.place.bees))
        # END Problem 6
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        nonmute = self.place.bees[:]
        if self.place.bees:
            bee_iter = iter(nonmute)
            for b in bee_iter:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.counter += 1
        self.armor = 1
        if QueenAnt.counter == 2:
            self.name = 'Imposter'
        else:
            self.name = QueenAnt.name
        self.dubbed = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.name == 'Queen':
            ...
            while exit:
                if exit.ant and exit.ant.is_container:
                    if exit.ant not in self.dubbed:
                        exit.ant.damage*=2
                        self.dubbed.append(exit.ant)
                    if exit.ant.contained_ant and exit.ant.contained_ant not in self.dubbed:
                        exit.ant.contained_ant.damage*=2
                        self.dubbed.append(exit.ant.contained_ant)
                elif exit.ant and exit.ant not in self.dubbed:
                    exit.ant.damage*=2
                    self.dubbed.append(exit.ant)
                exit = exit.exit
        else:
